...
from tools import mcp_tool_handlers, mcp_tool_loop
...
def _install_stub_server(mcp_tool_module, name: str, call_tool_impl):
    ...
    server._rpc_lock = mcp_tool_loop._run_on_mcp_loop(_make_lock, timeout=5)
...
def test_retry_helpers_return_the_functional_payload_verbatim(
    ...
    try:
        for helper, exc in (
            (mcp_tool_handlers._handle_auth_error_and_retry, OAuthFlowError("401")),
            (
                mcp_tool_handlers._handle_session_expired_and_retry,
...
def test_auth_retry_that_raises_still_reports_needs_reauth(
    ...
    try:
        out = mcp_tool_handlers._handle_auth_error_and_retry(
